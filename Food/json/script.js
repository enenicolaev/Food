...
function cards() {
  ...
  getCardInfo("http://localhost:3000/menu").then(data => {
    data.forEach(({
      img,
      altimg,
      title,
...
function forms(formSelector) {
  ...
  function bindPostData(form) {
    form.addEventListener('submit', e => {
      ...
      postData("http://localhost:3000/requests", json).then(response => {
...
window.addEventListener('DOMContentLoaded', function () {
  const modalTimerId = setTimeout(() => {
    Object(_modules_modal__WEBPACK_IMPORTED_MODULE_2__["openModal"])('.modal', modalTimerId);
  }, 30000);
